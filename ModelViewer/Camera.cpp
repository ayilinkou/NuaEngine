...
void Camera::GetProjMatrix(DirectX::XMMATRIX& ProjMatrix)
{
	...
	uint32_t FrameIndex = Application::GetSingletonPtr()->GetFrameIndex();
	if (FrameIndex != m_LastCalculatedFrame)
	{
		CalculateJitteredProjMatrix(FrameIndex);
	}

	ProjMatrix = m_ProjMatrixJittered;
}
...
float Camera::Halton(int index, int base) const
{
	float f = 1.f;
	float result = 0.f;
	while (index > 0)
...
void Camera::CalculateJitteredProjMatrix(uint32_t FrameIndex)
{
	m_LastCalculatedFrame = FrameIndex;
	...
	DirectX::XMStoreFloat4x4(&TempMatrix, m_ProjMatrix);

	TempMatrix._31 += JitterX;
	TempMatrix._32 += JitterY;

	m_ProjMatrixJittered = DirectX::XMLoadFloat4x4(&TempMatrix);
}
