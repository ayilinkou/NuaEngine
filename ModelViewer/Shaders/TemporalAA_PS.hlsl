...
{
    float2 MotionUV = VelocityTexture.Sample(PointSampler, p.TexCoord); // TODO: point sampler should be used here
    ...
    if (IsInRange(ReprojectedUV, 0.f, 1.f))
    {
        ...
        float4 HistoryPixel = HistoryTexture.Sample(PointSampler, ReprojectedUV);
